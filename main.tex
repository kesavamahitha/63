...
\centering
\(E(\hat{\theta}) = \theta\)\\
where, \(E(\hat{\theta})\) represents the expected value of the estimator \(\hat{\theta}\) and \(\theta\) represents the true parameter
\end{definition}
\begin{definition}
Cramer-rao bound can be defined as\\
\centering
\(Var($T$) \geq \frac{1}{I(\theta)}\)\\
where, \(I(\theta)\) represents a measure of the amount of information in the data about the parameter \(\theta\)
\end{definition}
\begin{definition}
...
\end{definition}
%Given, $T$ is an unbiased estimator of $\sigma^2$ whose variance attains the Cramer-Rao lower bound.
%\[\Rightarrow T \text{ is the Maximum Likelihood Unbiased Estimator}\]
